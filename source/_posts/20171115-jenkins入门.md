...
#### 添加一个Jenkins构建
* 在你的github项目里添加webhook:项目的settings中左边栏的webhooks,选择<code class="shortCode">add webhook</code>,<code class="shortCode">Payload URL</code>中填你的服务器地址(ip或域名)与jenkins端口再加上"/github-webhook",如<code class="shortCode">http:// 182.254.133.70:8080/github-webhook</code>
...
* Credentials:点击Add按钮添加一个Jenkins，Kind选择默认的<code class="shortCode">Username with password</code>将github的账号和密码填到相应的Username和Password中，点击Add完成添加。
...
### 赘述
#### flag
这是一个flag,如果没删的话就说明:如果你遇到了构建项目和webhook都配置正确,但是构建中的"GitHub hook trigger for GITScm polling"触发器选项一直提示你webhook有问题,那么你应该考虑删掉这个构建项目,重新配置一遍.