...
dockerfile我已经写好上传到GitHub仓库, 详情请见 👉 [传送门](https://github.com/VanjayDo/store/blob/master/docker-XmrStakJust4cpu/Dockerfile)

镜像已经构建完成并上传到DockerHub, 详情请见 👉 [传送门](https://hub.docker.com/r/vanjaydo/xmr-stak-just4cpu)

欢迎大家Star⭐我的项目, 谢谢😁.

## 使用方法
请在已经装好Docker的服务器上创建一个空文件夹, 就名为`xmr-stak`吧, 然后进入该文件夹, 运行命令`docker run -it -u $(id -u):$(id -g) --name xmr-stak -v "$PWD":/mnt vanjaydo/xmr-stak-just4cpu`(xmr-stak官方的docker image usage中命令比这个多了个`--rm`参数, 起作用是在容器退出的时候直接删除容器, 可能是考虑到安全方面的因素吧, 但是每次启动容器的时候都要重建一下, 不能直接start容器, 并不方便, 所以如果你认为有必要的话就在run后加上`--rm`, 为了启停方便的话就不加), 随后会进入容器的交互式对话, 下面一步步讲解: 
